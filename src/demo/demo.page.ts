// Self-contained HTML dashboard for the SCIM provisioning demo. Served at
// /demo; polls /demo/api/users (provisioned users) and /demo/api/events (the
// live actor-by-actor traffic log captured by the edge-proxy). The Okta setup
// guide is rendered from live server config so the values are always accurate.
...
// A copyable value pill used throughout the guide.
const val = (v: string) => `<code class="copy" data-v="${esc(v)}" title="Click to copy">${esc(v)}</code>`;

export function renderDemoPage(cfg: DemoConfig): string {
